 ...
 * @author kaitou
 */
public class PageQueryFrame<T extends BaseDomain> extends JFrame {
    /**
     * 操作列标题
     */
     ...
     * 标识数据存疑的属性名
     */
    protected static final String IN_DOUBT_FIELD_NAME = "inDoubt";
    /**
     * 当前frame
     */
    protected PageQueryFrame self = this;
    /**
     * 查询区域输入框动态集合
     */
     ...
     */
    public PageQueryFrame(IQueryObject<T> queryObject) {
    ...
    protected void initTableData() {
        ...
        recordCountShow.setText("条数：" + recordCount);
            ...
            public void run() {
                ...
                doRunWithWaiting(self, new OpRunnable() {
                    ...
                    public void run() {
                        ...
                        if (selectedRows.size() <= 0) {
                            exportList.addAll(queryAll(queryObject.domainType(), getConditions()));
                        } else {
                            for (int i = 0; i < selectedRows.size(); i++) {
